    ...
    <tbody>
        ...
        <tr>
            ...
            <td align="right">
                ...
                <?php
                    $sisa;
                    $query = $this->db->get_where('detail_penitipanemas', ['notitip' => $r->notitip, 'pilihan' => 1])->result();
                    $totalpenitipan = 0;
                    foreach ($query as $x) {
                        $totalpenitipan = $totalpenitipan + $x->jml;
                    }
                    echo number_format($totalpenitipan, 2, ",", ".");
                    ?>
            </td>
            <td align="right">
                <!-- Menampilkan Total Pengambilan -->
                <?php
                    $query = $this->db->get_where('detail_penitipanemas', ['notitip' => $r->notitip, 'pilihan' => 2])->result();
                    $totalpengambilan = 0;
                    foreach ($query as $x) {
                        $totalpengambilan = $totalpengambilan + $x->jml;
                    }
                    echo number_format($totalpengambilan, 2, ",", ".");
                    ?>
            </td>
            <td align="right">
                <?php
